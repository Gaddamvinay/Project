...
export class InnerWardComponent implements OnInit {
	...
	wearableDisplayedColumns: string[] = ['Wearable_serial_number','Battery_status','Wearable_status','Wearable_fall', 'Falls_activity','Night_walk_activity','Critically_low_battery_activity'];
				...
				let positions: any;
					  ...
					  if(kpi.itemCount > 0){
						...
						customerKpi.forEach(value => {
						  if(value.KPI){
							...
							Screen_time_usage = Screen_time_usage + this.transform(value.KPI.ScreenTimeUsage);
					...
					this.wearableTableData = filteredWearables.map(value => {
					  return {
						...
						registeredResidents: value.summary.reports &&  value.summary.reports.registeredResidents ? value.summary.reports.registeredResidents : 0,
						...
						registeredWearables: value.summary.reports && value.summary.reports.registeredWearables ? value.summary.reports.registeredWearables : 0,
					  }
					}).sort((a: any,b: any) => {
...
/*	getAllRooms(wardNo: string) {
		this.isLoading.room = true;
		this.commonHttp.getWardRoomDetails(wardNo, "fwadmin").subscribe((data: any) => {
			...
			data.forEach(room => {
				roomTableData.push({
					...
					Residents_Name: room.Residents.join(", "),
					Wearable_fall: room.Count,
					wearableFallIncreased: room.wearableFallInc,
  					fallDiff: room.wearableFallDiff
				});
			});
...
/*	getCareGiverList(orgId: string, wardId: string) {
	...
	/*getResident(orgId: string, wardId: string) {
