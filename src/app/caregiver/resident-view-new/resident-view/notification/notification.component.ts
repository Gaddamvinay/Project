...
@Component({
  selector: 'app-inner-notification',
...
export class NotificationComponent implements OnInit {
  ...
  openDialog() {
    this.dialog.open(ResidentSettingsComponent, {
      disableClose: true,
      width: '60vw',
      ...
      data: {
        payload: {
          ...
          notification_settings: {
            When_a_resident_has_not_come_back_to_bed_after_getting_up_at_night: false,
            how_often_is_the_information_sent_from_the_wearable: false,
            when_a_resident_fall_down: true,
            when_a_resident_walks_during_night_time: true,
            when_the_battery_less_than_7: true,
            when_the_battery_less_than_15: false
          }
        },
        type: 'resident'
      }
  ...
  constructor(private http: HttpClient, private commonHttp: CommonHttpService,
    private timeFormat: TimeFormatService, private common: CommonService,
    private residentService: ResidentService,
    private questionnireService: questionnireService, private dialog: MatDialog,
  ...
  */
  ...
  //           const stringifiedNotification = JSON.stringify(this.beforeFilterList)
  //           localStorage.setItem('notifications', stringifiedNotification);
  ...
  */
    ...
    this.commonHttp.getNotifications().subscribe((data: any) => {
      if (data.itemCount > 0) {
        this.commonHttp.getAllAlerts().subscribe((alert: any) => {
          ...
          const dbValues = data.body.filter(val => Object.keys(val.notification.notification_body || {}).length > 0).filter(val => val.details.resident_id === this.userId);
          this.notifications = dbValues.map(val => {
            this.count++;
            var content = '';
  ...
  changeSelection(startDate?: number, endDate?: number) {
    this.pageEnd = 30;
    this.isDataAvailable = false;
  ...
  public loadMore(pageEnd: any): void {
    ...
    if (this.pageEnd > this.count) {
      ...
      // else if(this.pageEnd>this.filteredList.length){
      //   this.isButtonVisible=false;
      // }
      else {
        this.isButtonVisible = true;
